...
async fn initialize_factory(app: AppHandle, window: Window) -> Result<(), String> {
    ...
    let has_python = Command::new(python_exe)
        .arg("--version")
        .output()
        .is_ok();
    ...
    if !bin_dest.join("magick.exe").exists() {
        window.emit("setup-progress", "Extracting core rendering binaries...").unwrap();
        let resource_bin = app.path().resolve("bin", BaseDirectory::Resource).map_err(|e| e.to_string())?;
    ...
    // 2. Create Venv
    window.emit("setup-progress", "Creating isolated neural environment...").unwrap();
    ...
    window.emit("setup-progress", "Installing AI dependencies (this may take a few minutes)...").unwrap();
    let req_path = app.path().resolve("requirements.txt", BaseDirectory::Resource).map_err(|e| e.to_string())?;
    run_command(pip_path.to_str().unwrap(), &["install", "-r", req_path.to_str().unwrap()], &app_dir)?;

    // 4. Install Playwright
    window.emit("setup-progress", "Configuring TikTok automation protocols...").unwrap();
    run_command(pip_path.to_str().unwrap(), &["install", "playwright"], &app_dir)?;
    
    let playwright_path = if cfg!(target_os = "windows") {
        app_dir.join("python_env").join("Scripts").join("playwright")
    } else {
        app_dir.join("python_env").join("bin").join("playwright")
    };
    run_command(playwright_path.to_str().unwrap(), &["install", "chromium"], &app_dir)?;

    // 5. Warm up Whisper
    window.emit("setup-progress", "Warming up local AI models...").unwrap();
    ...
    run_command(python_env_exe.to_str().unwrap(), &["-c", "import whisper; whisper.load_model('small')"], &app_dir)?;
...
async fn start_factory_server(app: AppHandle, state: State<'_, AppState>) -> Result<(), String> {
    ...
    let server_path = app.path().resolve("app_server.py", BaseDirectory::Resource).map_err(|e| e.to_string())?;

    let mut command = Command::new(python_exe);
    command.arg(server_path).current_dir(&app_dir);
